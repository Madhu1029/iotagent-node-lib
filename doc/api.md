...
-   Dynamic **Configuration API**: making use of the API URLs in the configuration URI, `/iot/configGroups` or
    `/iot/services`. Please, note that the configuration API manage servers under a URL that requires the `server.name`
    parameter to be set (the name of the IoT Agent we are using). If no name is configured `default` is taken as the
    default one.
...
Takes a configuration/service group body as the payload. The body does not have to be complete: for incomplete bodies,
just the existing attributes will be updated
